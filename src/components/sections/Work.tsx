...
const Project = ({ title, subtitle, description, impact, tags, link, index, gradient, icon }: ProjectProps) => {
  ...
  return (
    ...
    >
      <div className="relative p-6 md:p-8 rounded-3xl bg-card/50 backdrop-blur-sm border border-border/50 hover:border-accent/40 transition-all duration-500 hover:shadow-2xl hover:shadow-accent/10 overflow-hidden">
        ...
        <div className="relative">
          ...
          <div className="flex items-start justify-between mb-6">
            <div className="flex items-center gap-4">
              {/* Project icon with bounce animation */}
              <motion.div 
            ...
            <motion.div
              ...
              className="w-10 h-10 rounded-full border border-border/50 group-hover:border-accent/50 group-hover:bg-accent/10 flex items-center justify-center transition-all duration-300"
            >
              <svg
          ...
          {/* Impact highlight with pulse */}
          <motion.div 
            className={`inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r ${gradient} bg-opacity-10 mb-6`}
          ...
          <div className="flex flex-wrap gap-2">
            {tags.map((tag, tagIndex) => (
              <motion.span
                ...
                className="px-3 py-1.5 text-xs font-medium bg-background border border-border/80 rounded-full text-foreground/70 hover:border-accent/40 hover:text-accent transition-all duration-200 cursor-default"
              >
                {tag}
              </motion.span>
            ))}
    ...
    <section id="work" className="py-20 md:py-28 relative">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div 
          ...
          className="absolute top-1/4 right-0 w-[500px] h-[500px] rounded-full bg-gradient-to-bl from-accent/10 via-transparent to-transparent blur-3xl" 
        />
        <motion.div 
          animate={{ 
            x: [0, -30, 0],
          ...
          className="absolute bottom-1/4 left-0 w-[400px] h-[400px] rounded-full bg-gradient-to-tr from-blue-500/10 via-transparent to-transparent blur-3xl" 
        />
        ...
        {codeSymbols.map((symbol, i) => (
          <motion.span
            key={i}
            className="absolute text-accent/10 font-mono text-2xl select-none"
            style={{
              left: `${10 + (i * 12)}%`,
              top: `${20 + (i % 3) * 30}%`,
            }}
      ...
      <div className="max-w-[1400px] mx-auto px-6 relative z-10">
        ...
        >
          ...
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight max-w-4xl">
            ...
            <motion.span 
              className="bg-gradient-to-r from-accent via-blue-500 to-accent bg-clip-text text-transparent inline-block"
              animate={{ 
        ...
        >
          <motion.a
            ...
            whileTap={{ scale: 0.98 }}
            className="group inline-flex items-center gap-3 px-8 py-4 rounded-full border-2 border-foreground/20 text-foreground font-semibold hover:border-accent hover:text-accent transition-all duration-300 relative overflow-hidden"
          >
            <motion.div 
