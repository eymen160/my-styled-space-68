...
const Project = ({ title, subtitle, description, impact, tags, link, index, gradient, icon }: ProjectProps) => {
  ...
  return (
    ...
    >
      <div className="relative p-6 md:p-8 rounded-3xl bg-card/50 backdrop-blur-sm border border-border/50 hover:border-accent/40 transition-all duration-500 hover:shadow-xl hover:shadow-accent/10">
        {/* Hover gradient overlay */}
        <div className={`absolute inset-0 bg-gradient-to-br ${gradient} opacity-0 group-hover:opacity-[0.06] transition-opacity duration-500 rounded-3xl`} />
        ...
        <div className="relative">
          ...
          {/* Impact highlight */}
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r ${gradient} bg-opacity-10 mb-6`}>
          ...
          <div className="flex flex-wrap gap-2">
            {tags.map((tag, tagIndex) => (
              <motion.span
                ...
                className="px-3 py-1.5 text-xs font-medium bg-background border border-border/80 rounded-full text-foreground/70 hover:border-accent/40 transition-colors duration-200"
              >
                {tag}
              </motion.span>
            ))}
  ...
  return (
    <section id="work" className="py-24 md:py-40 relative">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-1/4 left-0 w-[400px] h-[400px] rounded-full bg-gradient-to-tr from-blue-500/10 via-transparent to-transparent blur-3xl" />
      </div>
      ...
      <div className="max-w-[1400px] mx-auto px-6 relative z-10">
        ...
        >
          <motion.a
            ...
            whileTap={{ scale: 0.98 }}
            className="inline-flex items-center gap-3 px-8 py-4 rounded-full border-2 border-foreground/20 text-foreground font-semibold hover:border-accent hover:text-accent transition-all duration-300"
          >
